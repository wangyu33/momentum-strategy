...
OUTPUT_DIR = RESEARCH_OUTPUT_DIR / "archive_flat" / "compare_energy_income_candidates"
...
def main() -> int:
    ...
    ax.plot(compare_df.index, compare_df["plus_energy"], linewidth=1.8, label="+ Energy")
    ax.plot(compare_df.index, compare_df["plus_resources"], linewidth=1.8, label="+ Resources")
    ax.plot(compare_df.index, compare_df["plus_hk_div_lowvol"], linewidth=1.8, label="+ HK Div LowVol")
    ax.plot(compare_df.index, compare_df["plus_global_dividend"], linewidth=1.8, label="+ Global Dividend")
    ax.plot(compare_df.index, compare_df["plus_energy_hk_div"], linewidth=1.8, label="+ Energy + HK Div")
    ax.plot(compare_df.index, compare_df["hs300_benchmark"], linewidth=1.6, linestyle="--", label="HS300 ETF")
    ax.set_title("Energy / Income ETF Candidate Comparison", loc="left", fontsize=16, fontweight="bold")
    ax.set_xlabel("Date")
