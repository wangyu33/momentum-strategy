...
OUTPUT_DIR, BEST_PATH, NOTIFY_STATE_PATH = build_archive_flat_strategy_paths("compare_stressbond_trigger_refine_rc184")
...
def main() -> int:
        ...
        for breadth_cut in breadth_candidates:
            for risk_cap in risk_cap_candidates:
                for short_ratio_cut in short_ratio_candidates:
                    ...
                    strategy = (
                        f"{current_context['strategy']}__trf2"
                    ...
                    append_variant_result(
                        ...
                        description=(
                            f"围绕当前 {treasury_row['name']} rc184 基线细化弱市切债触发："
    ...
    plot_lines = [
        ...
        (f"{current_context['strategy']}__trf2_vr900_vb070_rc182_srna_nav", "RC182", 1.8),
        (f"{current_context['strategy']}__trf2_vr900_vb070_rc184_srna_nav", "RC184", 1.8),
        (f"{current_context['strategy']}__trf2_vr900_vb070_rc184_sr920_nav", "RC184 SR92", 1.8),
    ]
    ...
    save_plot_and_print_baseline_preview(
        ...
        title="Stressbond Trigger Refine RC184 Comparison",
