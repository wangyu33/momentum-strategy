...
from compare_resource_guards import RESOURCE_ETF, run_variant, summarize
...
OUTPUT_DIR = RESEARCH_OUTPUT_DIR / "archive_flat" / "run_resource_abs08_backtest"
...
def save_outputs(selected: pd.DataFrame, prices: pd.DataFrame, result: pd.DataFrame, trades: pd.DataFrame, lookback: int) -> None:
    ...
    ax.set_title(f"Resource Abs08 NAV (lookback={lookback})", loc="left", fontsize=16, fontweight="bold")
    ax.set_xlabel("Date")
