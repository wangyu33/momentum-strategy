...
def build_confirmed_trade_reason(
    *,
    ...
    if extra_cap_triggered and base_exposure is not None and desired_exposure is not None:
        if strategy_id == "default" and current_momentum is not None:
            drawdown_text = format_ratio_percent(current_drawdown, digits=2) if current_drawdown is not None else "N/A"
            if current_momentum < DEFAULT_REGIME_MIX_OVERHEAT_MOMENTUM_CUT:
                previous_live_exposure = None
                if trade_previous_allocations:
                    previous_live_exposure = float(sum(weight for _, weight in trade_previous_allocations))
                from_exposure = previous_live_exposure if previous_live_exposure is not None else base_exposure
                direction = "加仓" if from_exposure is not None and desired_exposure > from_exposure + 1e-9 else "减仓"
                parts.append(
                    f"当前处于轻度预减仓区：信号动量 {format_ratio_percent(current_momentum, digits=2)} 已超过 "
                    f"{format_ratio_percent(DEFAULT_REGIME_MIX_PRE_OVERHEAT_END_CUT, digits=0)}，但尚未达到 "
                    f"{format_ratio_percent(DEFAULT_REGIME_MIX_OVERHEAT_MOMENTUM_CUT, digits=0)}；"
                    f"按连续仓位规则，风险仓位在 {format_ratio_percent(DEFAULT_REGIME_MIX_PRE_OVERHEAT_END_CUT, digits=0)}~{format_ratio_percent(DEFAULT_REGIME_MIX_OVERHEAT_MOMENTUM_CUT, digits=0)} "
                    f"区间继续从 {format_allocation_percent(DEFAULT_REGIME_MIX_PRE_OVERHEAT_END_EXPOSURE)} 线性降到 {format_allocation_percent(DEFAULT_REGIME_MIX_OVERHEAT_MAX_EXPOSURE)}，"
                    f"所以这次{direction}从 {format_allocation_percent(from_exposure)} 调整到 {format_allocation_percent(desired_exposure)}"
                )
            else:
                previous_live_exposure = None
                if trade_previous_allocations:
                    ...
                    previous_live_exposure = float(base_exposure)
                pre_stage_exposure_text = (
                    format_allocation_percent(previous_live_exposure)
                    if previous_live_exposure is not None
                    else format_allocation_percent(DEFAULT_REGIME_MIX_PRE_OVERHEAT_END_EXPOSURE)
                )
                parts.append(
                    f"当前处于连续过热降仓区：信号动量 {format_ratio_percent(current_momentum, digits=2)} 已超过 "
                    f"{format_ratio_percent(DEFAULT_REGIME_MIX_OVERHEAT_MOMENTUM_CUT, digits=0)}，且当前回撤 {drawdown_text} 高于 "
                    f"{format_ratio_percent(DEFAULT_REGIME_MIX_OVERHEAT_DRAWDOWN_CUT, digits=0)}；"
                    f"仓位先在 {format_ratio_percent(DEFAULT_REGIME_MIX_PRE_OVERHEAT_START_CUT, digits=0)}~{format_ratio_percent(DEFAULT_REGIME_MIX_PRE_OVERHEAT_END_CUT, digits=0)} "
                    f"区间预减到 {pre_stage_exposure_text}，再在 "
                    f"{format_ratio_percent(DEFAULT_REGIME_MIX_OVERHEAT_MOMENTUM_CUT, digits=0)}~{format_ratio_percent(DEFAULT_REGIME_MIX_OVERHEAT_HIGH_MOMENTUM_CUT, digits=0)} "
                    f"区间连续压缩，所以这次从 {pre_stage_exposure_text} 降到 {format_allocation_percent(desired_exposure)}"
                )
        else:
            parts.append(f"风控额外降仓 {format_allocation_percent(base_exposure)}->{format_allocation_percent(desired_exposure)}")
