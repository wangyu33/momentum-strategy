...
# 布林带过热压仓分两层，各自独立开关：值 >= 1.0 表示该层关闭。
# 普通层（cap）实测为纯负贡献：开启时以 0.50pp CAGR 换取 0.11pp 回撤，性价比差，故关闭。
DEFAULT_BOLL_HOT_CAP = 1.00
DEFAULT_BOLL_HOT_BANDWIDTH_PCT = 0.80
# 极热层单独保留，0.20~0.35 为平坦区（回撤 -16.6% 附近），取 0.30 居中。
DEFAULT_BOLL_EXTREME_HOT_CAP = 0.30
