...
4. 过热降仓
   - 正式默认基线不再启用弱市切债覆盖层
   - `511260 十年国债ETF` 仍保留在运行时池里，供研究脚本或后续覆盖层实验复用
   - 如果当前主信号落在防守池，且主信号自身历史动量分位 `>= 95%`
   - 则把总仓位进一步压到 `30%`
   - 同时保留高位过热保护：
...
- 所以正式默认基线的主动配置实际使用的是“`5` 只风险 + `3` 只防守”
- `511260` 会继续保留在运行时池里，但默认基线不会主动切入
...
6. 如果当前主信号落在防守池且该信号自身历史动量分位过热，则把总仓位进一步压低。
