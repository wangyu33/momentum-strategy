...
- `price_validation_issues.csv` 目前是“先显式暴露问题”，不是“自动修复所有问题”
  - 像 `159941` 的拆分型跳变，当前会被检测出来
  - 但如果不主动改复权逻辑，价格链本身不会被自动修正
