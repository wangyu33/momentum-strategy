...
- Boll 解耦（关普通层 + 极热层 `0.4225 -> 0.30`）的收益同样集中
  - 分年差：2019 `+2.43pp`、2020 `+5.43pp`、2025 `+4.71pp`、2026 `+1.29pp`
  - 2014-2016 是小幅负贡献（`-0.33` / `-0.55` / `-0.84pp`）
  - 全期 `+0.63pp` 年化、`+0.88pp` 回撤改善，但最大回撤区间积分略升 `+0.10`
  - 成本敏感性上候选在各档成本下都优于旧基线，分段上 2019 年后更优、2012-2018 略差
...
4. Boll 过热压仓（两层独立开关，`>= 1.0` 即关闭该层）
   - 触发前提：当前持仓价格触及 20 日布林上轨
   - 普通层（`DEFAULT_BOLL_HOT_CAP`，当前 `1.00` = 关闭）
     - 布林带宽 120 日百分位 `>= 80%` 时，仓位上限压到 `80%`
     - 实测纯负贡献：开启时用 `0.50pp` 年化换 `0.11pp` 回撤，性价比太差，故关闭
   - 极热层（`DEFAULT_BOLL_EXTREME_HOT_CAP`，当前 `0.30`）
     - 布林带宽 120 日百分位 `>= 90%` 时，仓位上限压到 `30%`
     - `0.20 ~ 0.35` 都是平坦区（回撤稳定在 `-16.6%` 附近），取 `0.30` 居中
   - 两层解耦前，`cap >= 1.0` 会被前置短路连带关掉极热层，两层无法独立配置
   - 解耦也顺带修掉了「极热层被普通层门槛嵌套」：旧实现里总仓位 `<= cap` 时极热层同样不生效
     在旧默认参数下只多 `2` 个触发日、期末净值 `+0.0175`，可忽略
