...
OUTPUT_DIR = RESEARCH_OUTPUT_DIR / "signal_stability_variants"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="对比在正式基线中引入波动/稳定性质量分后的效果。")
...
def build_variants() -> list[tuple[str, dict[str, object]]]:
    ...
    slope_vol_015 = build_default_strategy_params()
    slope_vol_015.update({"signal_volatility_penalty": 0.15})

    slope_vol_025 = build_default_strategy_params()
    slope_vol_025.update({"signal_volatility_penalty": 0.25})

    slope_downvol_025 = build_default_strategy_params()
    slope_downvol_025.update({"signal_downside_volatility_penalty": 0.25})

    slope_downvol_040 = build_default_strategy_params()
    slope_downvol_040.update({"signal_downside_volatility_penalty": 0.40})

    slope_r2_020 = build_default_strategy_params()
    slope_r2_020.update({"signal_r2_penalty": 0.020})

    slope_r2_035 = build_default_strategy_params()
    slope_r2_035.update({"signal_r2_penalty": 0.035})

    slope_downvol_r2 = build_default_strategy_params()
    slope_downvol_r2.update({"signal_downside_volatility_penalty": 0.25, "signal_r2_penalty": 0.020})

    return [
        ("baseline", baseline),
        ("slope_vol_015", slope_vol_015),
        ("slope_vol_025", slope_vol_025),
        ("slope_downvol_025", slope_downvol_025),
        ("slope_downvol_040", slope_downvol_040),
        ("slope_r2_020", slope_r2_020),
        ("slope_r2_035", slope_r2_035),
        ("slope_downvol025_r2_020", slope_downvol_r2),
    ]
...
def main() -> int:
    ...
    ax.set_title("Signal Stability Variants", loc="left", fontsize=16, fontweight="bold")
    ax.set_xlabel("Date")
