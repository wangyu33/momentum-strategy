...
OUTPUT_DIR = Path("international_ma_backtest/output")
...
STRATEGY_NAME = "gtaa_10m_sma_bil_monthly"

RISK_ASSETS = [
    {"role": "risk", "ticker": "SPY", "theme": "US Equity", "name": "SPDR S&P 500 ETF Trust"},
    {"role": "risk", "ticker": "EFA", "theme": "Developed ex-US", "name": "iShares MSCI EAFE ETF"},
    {"role": "risk", "ticker": "EEM", "theme": "Emerging Markets", "name": "iShares MSCI Emerging Markets ETF"},
    {"role": "risk", "ticker": "VNQ", "theme": "US REITs", "name": "Vanguard Real Estate ETF"},
    {"role": "risk", "ticker": "DBC", "theme": "Commodities", "name": "Invesco DB Commodity Index Tracking Fund"},
]
...
def fetch_price_panel(years: int) -> pd.DataFrame:
    ...
    frames = [fetch_one_us_etf_history(asset["ticker"], fetch_start) for asset in [*RISK_ASSETS, CASH_PROXY]]
...
def build_monthly_targets(prices: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    risk_symbols = [asset["ticker"] for asset in RISK_ASSETS]
    monthly_prices = prices.resample("M").last()
    monthly_sma = monthly_prices[risk_symbols].rolling(10).mean()
    monthly_signal = (monthly_prices[risk_symbols] > monthly_sma).fillna(False)

    target_weights = pd.DataFrame(0.0, index=monthly_prices.index, columns=prices.columns, dtype="float64")
    unit_weight = 1.0 / len(risk_symbols)

    for dt_idx in monthly_prices.index:
        signal_row = monthly_signal.loc[dt_idx]
        active_count = int(signal_row.sum())
        for symbol in risk_symbols:
            target_weights.loc[dt_idx, symbol] = unit_weight if bool(signal_row[symbol]) else 0.0
        target_weights.loc[dt_idx, CASH_PROXY["ticker"]] = 1.0 - active_count * unit_weight

    monthly_signal = monthly_signal.rename(columns=lambda symbol: f"signal_{symbol}")
    monthly_snapshot = pd.concat([monthly_prices.add_prefix("close_"), monthly_sma.add_prefix("sma10_"), monthly_signal], axis=1)
    return target_weights, monthly_snapshot
...
def build_yearly_returns(result: pd.DataFrame) -> pd.DataFrame:
    ...
    for year in years:
        ...
        period_index = nav.index[year_mask]
        if period_index.empty:
            continue
        start_idx = period_index[0]
        end_idx = period_index[-1]
        nav_start = float(nav.shift(1).loc[start_idx]) if pd.notna(nav.shift(1).loc[start_idx]) else 1.0
        nav_end = float(nav.loc[end_idx])
        bench_start = float(benchmark.shift(1).loc[start_idx]) if pd.notna(benchmark.shift(1).loc[start_idx]) else 1.0
        bench_end = float(benchmark.loc[end_idx])
        strategy_return = nav_end / nav_start - 1.0
...
def build_summary(result: pd.DataFrame, trades: pd.DataFrame) -> dict[str, float | int | str]:
    ...
    drawdown_integral = float((-drawdown.clip(upper=0.0)).sum())
    ...
    benchmark_sharpe = (
        float(benchmark_annualized_return / benchmark_annualized_vol) if benchmark_annualized_vol > 0 else float("nan")
    )
    ...
    return {
        ...
        "max_drawdown_integral": drawdown_integral,
        "benchmark_total_return": benchmark_total_return,
        "benchmark_annualized_return": benchmark_annualized_return,
        "benchmark_annualized_volatility": benchmark_annualized_vol,
        "benchmark_sharpe_rf0": benchmark_sharpe,
        "benchmark_max_drawdown": benchmark_max_drawdown,
        "trade_count": int(len(trades)),
        "avg_risk_exposure": float(result["risk_exposure"].mean()),
        "latest_risk_exposure": float(result["risk_exposure"].iloc[-1]),
...
def save_plots(result: pd.DataFrame) -> None:
    ...
    ax.plot(result.index, result["nav"], label="Strategy NAV", color="#1f4f82", linewidth=2.2)
    ax.plot(result.index, result["benchmark_nav"], label="Equal-Weight Risk Basket", color="#d2691e", linewidth=1.8)
    ax.set_title("International 10M SMA Strategy vs Benchmark", loc="left", fontsize=16, fontweight="bold")
    ax.set_xlabel("Date")
...
def run_strategy(prices: pd.DataFrame, years: int, cost_rate: float) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, dict[str, float | int | str]]:
    ...
    benchmark_return = returns[[asset["ticker"] for asset in RISK_ASSETS]].mean(axis=1)
    ...
    result["risk_exposure"] = daily_targets[[asset["ticker"] for asset in RISK_ASSETS]].sum(axis=1)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="运行国际均线策略回测。")
...
def main() -> int:
    ...
    asset_table = pd.DataFrame([*RISK_ASSETS, CASH_PROXY])
    ...
    write_dataframe_csv_atomic(asset_table, CORE_OUTPUT_DIR / "selected_assets.csv", index=False)
    write_dataframe_csv_atomic(prices_to_save, CORE_OUTPUT_DIR / "prices.csv")
    write_dataframe_csv_atomic(monthly_snapshot, CORE_OUTPUT_DIR / "monthly_signals.csv")
